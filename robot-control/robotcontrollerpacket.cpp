...
{
    quint8 messageTypeValue = m_packetData.at(0);
    m_packetId = static_cast<quint8>(m_packetData.at(1));

    if (messageTypeValue < 0xf0 ) {
        m_type = TypeResponse;
        m_command = static_cast<Command>(messageTypeValue);
        m_status = static_cast<Status>(m_packetData.at(2));
        m_payload = m_packetData.right(m_packetData.length() - 3);
    } else {
        m_type = TypeNotification;
        m_notification = static_cast<Notification>(messageTypeValue);
...
{
    return (m_command != RobotControllerPacket::CommandUnknown || m_notification != RobotControllerPacket::NotificationUnknown) && m_packetData.length() >= 2;
}
...
QDebug operator<<(QDebug debug, const RobotControllerPacket &packet)
{
    ...
    case RobotControllerPacket::TypeRequest:
        ...
        debug.nospace() << packet.command() << ", ";
        debug.nospace() << "id: " << RobotSerialInterface::byteToHexString(packet.packetId())  << " (" << packet.packetId() << ")";
    ...
    case RobotControllerPacket::TypeNotification:
        ...
        debug.nospace() << packet.notification() << ", ";
        debug.nospace() << "id: " << RobotSerialInterface::byteToHexString(packet.packetId())  << " (" << packet.packetId() << ")";
        if (!packet.payload().isEmpty()) {
            debug.nospace() << ", " << packet.payload().toHex();
            if (packet.notification() == RobotControllerPacket::NotificationDebugMessage) {
                debug.nospace() << " (" << QString::fromUtf8(packet.payload()) << ")";
            }
        }
